...
export const getPriceForTokenWithRouteFeatureWithThreePath = async (
  ...
  if (wallet.signer !== 'signer') {
    ...
    const value1 = await rout.getAmountsOut(checkPriceOfOne, [
      routeAddress[0],
      routeAddress[1],
    ]);

    const value1string = await value1.toString().split(',');

    const getValue =
      routeAddress[0] === routeAddress[2]
        ? '1000000000000000000,1000000000000000000'
        : await rout.getAmountsOut(value1string[1], [
            routeAddress[1],
            routeAddress[2],
          ]);
    return getValue.toString();
...
export const getPriceForTokenWithRouteFeatureWithFourPath = async (
  ...
  if (wallet.signer !== 'signer') {
    ...
    const value2 = await rout.getAmountsOut(value1string[1], [
      routeAddress[1],
      routeAddress[2],
    ]);

    const value2string = await value2.toString().split(',');

    const getValue =
      routeAddress[0] === routeAddress[3]
        ? '1000000000000000000,1000000000000000000'
        : await rout.getAmountsOut(value2string[1], [
            routeAddress[2],
            routeAddress[3],
          ]);
    return getValue.toString();
