...
export function LiquidityPage(props) {
  ...
  useEffect(async () => {
    ...
    if (
      parseInt(fromValue) > parseInt(fromSelectedToken.balance) ||
      parseInt(toValue) > parseInt(toSelectedToken.balance)
    ) {
      ...
      if (wallet.address != '0x') {
        try {
          ...
          const liquidityValue =
            liquidities.length && liquidities[0].path[0].fromPath;
          const convertValue = ethers.utils.formatEther(
            liquidityValue,
            'ether',
          );
          const stateValue =
            Number((convertValue.toString() * percentValue) / 100) / 1e18;
  ...
  const setUpUrl = () => {
    if (
      ...
      const toTokenURL = checkIfTokenIsListed(fromSelectedToken.symbol)
        ? fromSelectedToken.symbol
        : fromSelectedToken.address;
      const fromTokenURL = checkIfTokenIsListed(toSelectedToken.symbol)
  ...
  const checkIfLiquidityPairExist = async (
    fromAddress = fromSelectedToken.address,
    toAddress = toSelectedToken.address,
  ) => {
  ...
  const getAllLiquidities = async () => {
    ...
    try {
      ...
      const pairsData = await Promise.all(
        allExchange.map(address => getPoolData(address)),
      );
  ...
  const approveToToken = async () => {
    try {
      if (!isNotEmpty(toSelectedToken)) {
        ...
        const approveResponse = await approveToken(
          wallet.address,
          toSelectedToken.address,
          wallet.signer,
          balance,
      ...
      try {
        ...
        const gasCost = await rout.estimateGas.addLiquidityETH(
          ...
          ethers.utils.parseEther(
            (amountTokenDesired * 0.8).toString(),
            'ether',
          ),
 ...
 props.showErrorMessage(msg);
  ...
  async function approveSmartSwapLPTokens(LPTokenAddress) {
    if (wallet.signer !== 'signer') {
      try {
        ...
        if (
          typeof approveTransaction.hash !== 'undefined' &&
          confirmations >= 2 &&
          status
        ) {
  ...
  const getAllCurrentLiquities = async () => {
    ...
    const liquid = liquidities.filter(
      liquid =>
        (liquid.path[0].token === fromSelectedToken.symbol &&
          liquid.path[1].token === toSelectedToken.symbol) ||
        (liquid.path[0].token === toSelectedToken.symbol &&
          liquid.path[1].token === fromSelectedToken.symbol),
    )[0];
    closeInput();
    if (liquid) {
  ...
  const closeAllModals = async (val = false) => {
    setPopupText(
      "Waiting for confirmation from metamask. It is better you don't leave this page",
    );
  ...
  const confirmingSupply = () => {
    if (
      ...
      (fromSelectedToken.symbol === 'BNB' && toSelectedToken.symbol !== 'BNB')
    ) {
      addingLiquidityForETH();
    } else if (
      (fromSelectedToken.symbol === 'RGP' &&
        toSelectedToken.symbol !== 'RGP') ||
      (fromSelectedToken.symbol === 'RGP' && toSelectedToken.symbol === 'WBNB')
    ) {
      addingLiquidity();
    } else {
      addingLiquidity();
  ...
  const removeALiquidity = async pairAddress => {
    const liquidity = liquidities.filter(
      liquidity => liquidity.pairAddress === pairAddress,
    );
  ...
  async function calculateToValue(inputAmount = fromValue, field = 'from') {
    if (
      ...
      try {
        ...
        const [tokenAReserve, tokenBreserve] = await LPcontract.getReserves();
        const liqidityRatio =
          tokenAReserve.toString() / tokenBreserve.toString();
  ...
  async function RGPCheckAllowance() {
    if (wallet.signer !== 'signer') {
      ...
      const check = await rgp.allowance(wallet.address, SMART_SWAP.MasterChef, {
        from: wallet.address,
      });
      return check;
    }
  ...
  async function BUSDcheckAllowance() {
    if (wallet.signer !== 'signer') {
      const busd = await BUSDToken();
      return await busd.allowance(wallet.address, SMART_SWAP.MasterChef, {
        from: wallet.address,
      });
    }
  }
